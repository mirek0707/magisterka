...
class BooksSpider(scrapy.Spider):
    ...
    def errback(self, failure):
        ...
        if request.meta['proxy'] == self.proxy_pool[self.proxy_index % len(self.proxy_pool)]:
            self.proxy_index += 1
        ...
        self.logger.error(
            '!!!!!!!!!!!!!!!!!%s - Changing proxy to %d', request.url, self.proxy_index % len(self.proxy_pool))
        yield scrapy.Request(request.url, callback=self.parse, errback=self.errback, meta={'q': request.url.split('&q=')[1], 'key': self.api_key, 'proxy': self.proxy_pool[self.proxy_index % len(self.proxy_pool)]})
