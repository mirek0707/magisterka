...
class CreateBookModel(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                "title": "W grze",
                "author": ["Jerzy Brzęczek", "Małgorzata Domagalik"],
                "pages": 352,
                "isbn": "9788328075337",
...
class UpdateBookModel(BaseModel):
    ...
    @field_validator(
        "pages", "ratings_tk_number", "ratings_gr_number", "ratings_lc_number"
    )
    @classmethod
    def validate_int(cls, field_val: int, info: ValidationInfo) -> int:
        if field_val < 0:
    ...
    @classmethod
    def validate_float(cls, field_val: float, info: ValidationInfo) -> float:
        if field_val < 0:
            raise ValueError(f"Number of {info.field_name} is not valid")
